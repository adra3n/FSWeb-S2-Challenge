...
/* GÖREV 2:
		paragrafOlustur fonksiyonuna aşağıdakileri uygulayın.
			...
			5. Oluşturulan bu dizinin tek sayılı anahtarlarını(index) [1,3,5,7,9] kullanarak ilk 5 elemanı geriçağırım olarak çağırılan cumleKur dizisinin parametreleri olarak aktarılarak çağırılacak 
...
function paragrafOlustur(arr, cumleKurCB, cumlelereDonusturCB){
	const dizi = cumlelereDonusturCB(arr, " ");
	// const tekSayiDizi = dizi.filter((item)=>{
	// 	return item i % 2 !== 0;
	// })
	const tekSayiDizi=[]
	for(let i = 1; i<dizi.length; i+=2){
	tekSayiDizi.push(dizi[i]);
	}

	console.log(tekSayiDizi)
	return cumleKurCB(tekSayiDizi[0],tekSayiDizi[1],tekSayiDizi[2],tekSayiDizi[3],tekSayiDizi[4])
	}
 ...
 */
// let metin = "Ben iyi bir iyi yazılımcıyım.";


// let yeniMetin = metin.replaceAll("iyi", "süper");
// console.log(metin);
...
// let mesajArray = []
// mesajArray= mesaj.split(" ") 
// 	for (let key in object){
		
// 		if(mesajArray.includes(key.toLowerCase()) || mesajArray.includes(key.toUpperCase()) ){

// 		// mesajArray[i]= mesaj.replace(mesaj, object[key])
// 		}
// 	}
